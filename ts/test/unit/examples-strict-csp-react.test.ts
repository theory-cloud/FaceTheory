...
test(
  'vite strict CSP react example: renders external assets and framework SSR hydration sidecars',
  { concurrency: false },
  async () => {
    const cwd = path.resolve('.');

    const distDir = path.resolve('examples/vite-strict-csp-react/dist');
    await rm(distDir, { recursive: true, force: true });

    await execFileAsync(
      'npm',
      ['run', 'example:vite:react:strict-csp:build'],
      { cwd },
    );

    const manifestPath = path.resolve(
      'examples/vite-strict-csp-react/dist/client/.vite/manifest.json',
    );
    ...
    const serverEntryPath = path.resolve(
      'examples/vite-strict-csp-react/dist/server/entry-server.js',
    );
    assert.ok(await exists(serverEntryPath));

    const serverMod = await import(pathToFileURL(serverEntryPath).href);
    const app = serverMod.createViteStrictCspReactExampleApp(manifest);
    ...
    assert.ok(body.includes('FaceTheory Strict CSP React'));
    assert.ok(body.includes('React + Vite without inline output'));
    ...
    assert.deepEqual(
      ...
      serverMod.strictCspReactDataForPath('/'),
    );
    ...
    for (const injectedPath of injectedPaths) {
      const builtPath = path.resolve(
        'examples/vite-strict-csp-react/dist/client',
        `.${injectedPath}`,
      );
