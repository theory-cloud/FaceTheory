...
```bash
export FACETHEORY_VERSION=4.0.6-rc # x-release-please-version
...
| `@theory-cloud/facetheory/control-plane`                | Control-plane presets          | Host-owned control-plane contracts, section delivery presets, guard states, and boundary helpers.                                                       |
...
| `@theory-cloud/facetheory/stitch-tokens`                | Shared Stitch token utilities  | `StitchTokenSet`, `StitchCssVarOptions`, `stitchToCssVars`, and `stitchCssVarsToRootBlock`.                                                             |
| `@theory-cloud/facetheory/stitch-shell`                 | Shared Stitch shell contracts  | `NavItem`, `BreadcrumbNode`, `ResolvedNav`, `CalloutVariant`, and `resolveActiveNav`.                                                                   |
| `@theory-cloud/facetheory/stitch-hosted-auth`           | Shared hosted-auth contracts   | Framework-neutral hosted-auth layout, flow-step, OTP, consent, and state-card contracts.                                                                |
| `@theory-cloud/facetheory/stitch-admin`                 | Shared Stitch admin contracts  | Dense-admin data contracts for tabs, filters, logs, status, authority, guard state, metadata, health, visibility, and explicit empty states.            |
| `@theory-cloud/facetheory/react`                        | React adapter                  | `renderReact`, `renderReactStream`, `createReactFace`, and `createReactStreamFace`.                                                                     |
...
| `@theory-cloud/facetheory/react/stitch-shell`           | React Stitch shell primitives  | `Shell`, `Sidebar`, `Topbar`, `BrandHeader`, page/section/panel/stat/callout primitives, and `resolveActiveNav`.                                        |
| `@theory-cloud/facetheory/react/stitch-hosted-auth`     | React hosted-auth primitives   | React `AuthPageLayout`, `AuthCard`, flow, passkey, OTP, consent, and auth-state primitives.                                                             |
| `@theory-cloud/facetheory/react/stitch-admin`           | React dense-admin primitives   | React data table, detail/form/status/destructive/tabs/filter/log, metadata, operator, health, visibility, wizard, package-source, and audit primitives. |
| `@theory-cloud/facetheory/react/responsive-primitives`  | React responsive primitives    | React `ChoiceCard`, selectable card grid, responsive links/buttons, and related a11y helpers.                                                           |
| `@theory-cloud/facetheory/vue`                          | Vue adapter                    | `renderVue`, `renderVueStream`, `createVueFace`, `createVueStreamFace`, and re-exported `h`.                                                            |
| `@theory-cloud/facetheory/vue/responsive-primitives`    | Vue responsive primitives      | Vue `ChoiceCard`, selectable card grid, responsive links/buttons, and related a11y helpers.                                                             |
| `@theory-cloud/facetheory/vue/stitch-shell`             | Vue Stitch shell primitives    | Vue `Shell`, `Sidebar`, `Topbar`, `BrandHeader`, page/section/panel/stat/callout primitives, and `resolveActiveNav`.                                    |
| `@theory-cloud/facetheory/vue/stitch-hosted-auth`       | Vue hosted-auth primitives     | Vue `AuthPageLayout`, `AuthCard`, flow, passkey, OTP, consent, and auth-state primitives.                                                               |
| `@theory-cloud/facetheory/vue/stitch-admin`             | Vue dense-admin primitives     | Vue dense-admin, metadata, operator, health, visibility, wizard, package-source, and audit primitives.                                                  |
| `@theory-cloud/facetheory/svelte`                       | Svelte adapter                 | `renderSvelte` and `createSvelteFace`.                                                                                                                  |
| `@theory-cloud/facetheory/svelte/responsive-primitives` | Svelte responsive primitives   | Svelte `ChoiceCard`, selectable card grid, responsive links/buttons, and related a11y helpers.                                                          |
| `@theory-cloud/facetheory/svelte/stitch-shell`          | Svelte Stitch shell primitives | Svelte `Shell`, `Sidebar`, `Topbar`, `BrandHeader`, page/section/panel/stat/callout primitives, and `resolveActiveNav`.                                 |
| `@theory-cloud/facetheory/svelte/stitch-hosted-auth`    | Svelte hosted-auth primitives  | Svelte `AuthPageLayout`, `AuthCard`, flow, passkey, OTP, consent, and auth-state primitives.                                                            |
| `@theory-cloud/facetheory/svelte/stitch-admin`          | Svelte dense-admin primitives  | Svelte dense-admin, metadata, operator, health, visibility, wizard, package-source, and audit primitives.                                               |
...
| ------------------- | ------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `FaceModule<TData>` | Route definition                     | Uses `route`, `mode`, optional `load`, optional `generateStaticParams`, optional `revalidateSeconds`, and `render`. `load` returns `TData` and `render(ctx, data)` receives that same type; `defineFace()` infers it from the loader. `createFaceApp()` rejects empty routes, non-function `render`, modes outside `ssr`, `ssg`, or `isr`, ISR Faces without `revalidateSeconds`, and parameterized SSG Faces without `generateStaticParams()`. SSG params must resolve to normal route segments; dot-segments such as `.` and `..` are rejected. |
...
| `FaceHeaders`       | Normalized HTTP headers              | Canonical FaceTheory header map: `Record<string, string[]>`. Import `FaceHeaders` for request/response header shapes; the legacy `Headers` alias was removed in v4.0.0.                                                                                                                                                                                                                                                                                                                                                                           |
| `FaceRequest`       | Normalized request input             | Supports `FaceHeaders`, cookies, query, body, base64 marker, and optional `cspNonce`.                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `FaceResponse`      | Runtime response                     | Includes normalized `FaceHeaders`, cookies array, status, body, and `isBase64`. Resource helpers return this shape directly for JSON/text/empty/method-not-allowed routes.                                                                                                                                                                                                                                                                                                                                                                        |
| `FaceRenderResult`  | Render output before HTTP conversion | Supports document-shell attrs (`lang`, `htmlAttrs`, `bodyAttrs`), `head.title`, `headTags`, `styleTags`, `csp`, `html`, cookies, headers, and hydration payload. Use `head.title` for the page title and structured `headTags` / `styleTags` for all other head output.                                                                                                                                                                                                                                                                           |
| `FaceContext`       | Per-request context                  | Exposes normalized request, route params, and proxy match.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `FaceAppOptions`    | App constructor options              | Accepts `faces`, optional `canonicalOrigin` for explicit strict head URL validation, optional raw `resources`, optional framework-owned SSR hydration sidecars, optional ISR config, optional observability hooks, and optional strict-CSP runtime limits.                                                                                                                                                                                                                                                                                        |
...
- `titleTag`, `metaTag`, `openGraph`, `twitterCard`, `canonical`, and `jsonLd` are helper-first authoring APIs that return normal `FaceHeadTag` values. They compose through the same deterministic ordering and de-duplication path as hand-authored tags.
- Keyless meta/link/script/style tags now de-duplicate structurally: identical rendered tags collapse with last-wins ordering, while distinct JSON-LD blocks or other distinct structured tags remain present. Add an `id`, `name`, `property`, `rel`/`href`, `src`, or `data-emotion` when you need a semantic de-duplication key.
- `stitchCssVarsToRootBlock()` returns CSS text, not a full `<style>` tag, and escapes `</style>` terminators as defense-in-depth. Feed that string into `styleTags` or a `headTags` style entry rather than wrapping it in caller-owned raw head HTML.
...
Header maps use the `FaceHeaders` name to avoid colliding with the browser
`Headers` class. v4.0.0 removes the legacy `Headers` type alias; update
consumer imports to `FaceHeaders`.
