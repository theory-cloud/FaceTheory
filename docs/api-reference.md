...
| `FaceHeaders`       | Normalized HTTP headers              | Canonical FaceTheory header map: `Record<string, string[]>`. Import `FaceHeaders` for request/response header shapes; the legacy `Headers` alias remains exported but deprecated until the v4.0.0 item 51 removal.                                                                                                                                                                                                 |
...
- Keyless tags are intentionally exempt from de-duplication. This preserves multiple JSON-LD blocks and other repeatable structured tags until the structural de-dup item lands; add an `id` when you want a script/style tag to use normal last-wins de-duplication.
...
`Headers` class. `Headers` is still exported as a deprecated compatibility alias
for the 3.x train and is scheduled for removal in v4.0.0 item 51.
